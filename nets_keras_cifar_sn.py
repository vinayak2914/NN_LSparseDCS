...
import numpy as np
import SNConv as sn
# from NN_LSparseDCS.SNConv import ConvSN2D
...
class SNConvMesNet(tf.keras.Model):
    def __init__(self, num_outputs=2, name='snconv_measure'):
        ...
        self.conv2d_6 = sn.ConvSN2D(256, (4, 4), strides=(2, 2), padding='same')
        self.actv_6 = tf.keras.layers.LeakyReLU(alpha=0.1)

        self.conv2d_7 = sn.ConvSN2D(512, (3, 3), strides=(1, 1), padding='same')
        ...
        self.dense_out = sn.DenseSN(num_outputs)



    def call(self, inputs):
        conv1_out = self.conv2d_1(inputs)
        # conv1_out = sn.ConvSN2D(64, kernel_size=3, strides=1, padding='same')(inputs)
