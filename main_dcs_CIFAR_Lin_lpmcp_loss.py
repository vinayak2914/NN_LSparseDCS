...
def get_rep_loss(img1, img2, measure_net):
    ...
    img_diff_norm = tf.norm(tf.keras.layers.Flatten()(img1) - tf.keras.layers.Flatten()(img2), axis=-1)
...
def get_measurement_error(target_meas, sample_img, measure_net):
    ...
    return tf.reduce_sum(
        tf.square(tf.keras.layers.Flatten()(measure_net(tf.keras.layers.Flatten()(sample_img), meas_mtx)) -
...
def optimization_loss_Sparse(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
    ...
    r1_Sparse = get_rep_loss(tf.keras.layers.Flatten()(optimized_sample_Sparse),
                             tf.keras.layers.Flatten()(initial_sample_Sparse), measureSparse_net)
    r2_Sparse = get_rep_loss(tf.keras.layers.Flatten()(optimized_sample_Sparse),
                             tf.keras.layers.Flatten()(x_batch_copy), measureSparse_net)
    r3_Sparse = get_rep_loss(tf.keras.layers.Flatten()(initial_sample_Sparse), tf.keras.layers.Flatten()(x_batch_copy),
                             measureSparse_net)
...
def optimization_loss_l0(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
...
def optimization_loss_l1(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
...
def optimization_loss_lp(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
...
def optimization_loss_mcp(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
...
def optimization_loss(z_i_Sparse, x_batch_copy, generatorSparse_net, measureSparse_net, meas_mtx):
    meas_img_Sparse = measureSparse_net(tf.keras.layers.Flatten()(x_batch_copy), meas_mtx)
...
with tf.device('/GPU:0'):
    ...
    if load_weights == 1:
        ...
        generatorl0_net = nets.SNConvGenNetll()

        generatorl1_net = nets.SNConvGenNetl()

        generatorlp_net = nets.SNConvGenNetlp()

        generatormcp_net = nets.SNConvGenNetmcp()
        ...
        load_sample_l1 = generatorl1_net(load_l1_z, is_training=True)
        ...
        meas_mtx = tf.random.normal(
            shape=[1, num_measurements, train_dataset.shape[1] * train_dataset.shape[2] * train_dataset.shape[3]],
            mean=0.0, stddev=1 / num_measurements, dtype=tf.float32)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for step in range(bat_per_epo):
            ...
            with tf.GradientTape() as tapeSparse:
                z_i_Sparse = tf.identity(generator_inputs_sparse)

                total_loss_Sparse, generator_loss_Sparse, recont_loss_Sparse, optimised_z_Sparse = optimization_loss_Sparse(
                    z_i_Sparse, x_batch_copy, generatorSparse_net, measure_net, meas_mtx)
            ...
            with tf.GradientTape() as tape:
                ...
                total_loss, generator_loss, recont_loss, optimised_z = optimization_loss(z_i, x_batch_copy,
                                                                                         generator_net,
    ...
    for step in range(bat_test_per_epo):
        ...
        # reconstructions_Sparse_tst = generatorSparse_net(optimised_z_Sparse_tst,is_training = False)
        l1DCS_test_reconstloss_itr = tf.reduce_mean(tf.norm(tf.keras.layers.Flatten()(reconstructionss_l1_tst) -
                                                            tf.keras.layers.Flatten()(x_batch_test), axis=-1))
    ...
    reconstructions_l0_tst_np_reshape = tf.reshape(reconstructions_l0_tst_np,
                                                   [batch_size, data_dim, reconstructions_Sparse_tst_np.shape[3]])
    reconstructions_l1_tst_np_reshape = tf.reshape(reconstructions_l1_tst_np,
                                                   [batch_size, data_dim, reconstructions_Sparse_tst_np.shape[3]])
    reconstructions_lp_tst_np_reshape = tf.reshape(reconstructions_lp_tst_np,
                                                   [batch_size, data_dim, reconstructions_Sparse_tst_np.shape[3]])
    reconstructions_mcp_tst_np_reshape = tf.reshape(reconstructions_mcp_tst_np,
                                                    [batch_size, data_dim, reconstructions_Sparse_tst_np.shape[3]])
    data_np_reshape = tf.reshape(data_np, [batch_size, data_dim, reconstructions_Sparse_tst_np.shape[3]])
