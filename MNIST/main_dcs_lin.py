...
def get_rep_loss(img1, img2, measure_net):
    ...
    # m1 = measure_net(img1)
    m1 = tf.linalg.matmul(img1, measure_net.T)
    m2 = tf.linalg.matmul(img2, measure_net.T)
    # m2 = measure_net(img2)
...
def get_measurement_error(target_meas, sample_img, measure_net):
    ...
    return tf.reduce_sum(tf.square(tf.linalg.matmul(sample_img,measure_net.T) - target_meas), -1)
...
# Measurement matrix A(i,j) ~ N(0,1/m)
def gen_random_measmatrix(n,m):
    measure_matrix = np.sqrt(1/m) * np.random.randn(m, n)
...
measure_matrix = gen_random_measmatrix(data_dim,num_measurements)
...
for epoch in range(140,epochs):
    ...
    for step in range(bat_per_epo):
        ...
        with tf.GradientTape() as tapeSparse:
            ...
            meas_img_Sparse = tf.linalg.matmul(x_img_reshape,measure_matrix.T)
            ...
            generator_loss_Sparse = tf.reduce_mean(
                gen_loss_fn(meas_img_Sparse, optimized_sample_Sparse, measure_matrix))
            recont_loss_Sparse = tf.reduce_mean(tf.norm(optimized_sample_Sparse - x_img_reshape, axis=-1))
            r1_Sparse = get_rep_loss(optimized_sample_Sparse, initial_sample_Sparse, measure_matrix)
            r2_Sparse = get_rep_loss(optimized_sample_Sparse, x_img_reshape, measure_matrix)
            r3_Sparse = get_rep_loss(initial_sample_Sparse, x_img_reshape, measure_matrix)
        ...
        with tf.GradientTape() as tape:
            ...
            meas_img = tf.linalg.matmul(x_img_reshape, measure_matrix.T)
            optimised_z = optimise_and_sample(z_i, meas_img, generator_net, measure_matrix)
            optimized_sample = generator_net(optimised_z)
            initial_sample = generator_net(z_i)
            generator_loss = tf.reduce_mean(gen_loss_fn(meas_img, optimized_sample, measure_matrix))
            recont_loss = tf.reduce_mean(tf.norm(optimized_sample - x_img_reshape, axis=-1))
            r1 = get_rep_loss(optimized_sample, initial_sample, measure_matrix)
            r2 = get_rep_loss(optimized_sample, x_img_reshape, measure_matrix)
            r3 = get_rep_loss(initial_sample, x_img_reshape, measure_matrix)
    ...
    for step in range(bat_valid_per_epo):
        ...
        meas_img_Sparse_v = tf.linalg.matmul(x_img_reshape_v, measure_matrix.T)
...
for step in range(bat_test_per_epo):
    ...
    meas_img_Sparse_tst = tf.linalg.matmul(x_img_reshape_tst, measure_matrix.T)
...
scipy.io.savemat(mat_file, mdict={'measure_matrix':measure_matrix,'DCS_test_reconstloss': DCS_test_reconstloss,
