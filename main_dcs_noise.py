...
def optimise_and_sample_Sparse(init_z, measure_data, generator_net, measure_net,is_training):
    if num_z_iters == 0:
        z_final = init_z
    else:
        ...
        def loop_body(i, z):
            with tf.GradientTape() as tape:
                ...
                tape.watch(z)
                gen_loss = gen_loss_fn(measure_data, loop_samples, measure_net)
...
for epoch in range(epochs):
    ...
    for step in range(bat_valid_per_epo):
        ...
        optimised_z_Sparse_v = optimise_and_sample_Sparse(z_i_Sparse_v, meas_img_Sparse_v, generatorSparse_net,
                                                          gaussian_noise_layer,is_training=False)
        ...
        optimised_z_v = optimise_and_sample(z_i_v, meas_img_v, generator_net, gaussian_noise_layer,is_training=False)
...
# Model Saving
    if (epoch + 1) % 10 == 0:
        filename1 = os.getcwd()+'/DeepcsTF2_%d_%d/gen_n_%d_%d_%d_%04d' % (num_measurements,num_latents,num_measurements,num_latents,dim_latent,epoch)
        filename2 = os.getcwd()+'/DeepcsTF2_%d_%d/meas_n_%d_%d_%d_%04d' % (num_measurements,num_latents,num_measurements,num_latents,dim_latent,epoch)
        filename3 = os.getcwd()+'/DeepcsTF2_%d_%d/genSpar_n_%d_%d_%d_%04d' % (num_measurements,num_latents,num_measurements,num_latents,dim_latent,epoch)
        filename4 = os.getcwd()+'/DeepcsTF2_%d_%d/measSpar_n_%d_%d_%d_%04d' %(num_measurements,num_latents,num_measurements,num_latents,dim_latent,epoch)
...
for step in range(bat_test_per_epo):
    ...
    optimised_z_tst = optimise_and_sample(z_i_test, meas_img_tst, generator_net, gaussian_noise_layer,is_training=False)
