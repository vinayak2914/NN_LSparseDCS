...
lambda_l1 = tf.Variable(tf.exp(math.log(0.01)), dtype=tf.float32)
rand_seed = 14
optimizer = tf.keras.optimizers.Adam(1e-4)
tf.random.set_seed(rand_seed)
seed(rand_seed)
output_dir = ('DeepcsTF2_z%d_m%d_spr%d' % (num_z_iters, num_measurements, num_latents))
...
def optimization_loss_l0(z_i_Sparse, x_img_reshape, generatorSparse_net, measureSparse_net):
    ...
    optimised_z_Sparse = optimise_and_sample_Sparse(z_i_Sparse, meas_img_Sparse, generatorSparse_net,
                                                    measureSparse_net)
    indices_to_remove = tf.math.less(optimised_z_Sparse,lambda_l1)
    ...
    optimised_z_Sparse = tf.math.multiply(indices_float, optimised_z_Sparse)
...
def optimise_and_sample_Sparse(init_z, data, generator_net, measure_net):
    if num_z_iters == 0:
        ...
        def loop_body(i, z):
            ...
            # # indices_to_remove = tf.math.less(z,lambda_l1)
            # # # indices_to_remove = tf.abs(z) >= tf.math.top_k(tf.abs(z), num_latents)[0][..., -1, None]
            # # indices_float = tf.dtypes.cast(indices_to_remove, tf.float32)
...
for epoch in range(start_epoch, epochs):
    ...
    for step in range(bat_per_epo):
        ...
        with tf.GradientTape() as tapeSparse:
            ...
            z_i_Sparse = tf.identity(generator_inputs)
            ...
            total_loss_Sparse, generator_loss_Sparse, recont_loss_Sparse, optimised_z_Sparse = optimization_loss_l0(
                z_i_Sparse, x_img_reshape, generatorSparse_net, measureSparse_net)
            ...
            train_var_Sparse.append(lambda_l1)
...
mat_file = os.getcwd() + '/' + output_dir + '/Final_saved_%d_%d_%d.mat' % (num_measurements, num_latents, dim_latent)
scipy.io.savemat(mat_file, mdict={'total_loss_Sparse_itr_mean': total_loss_Sparse_itr_mean,
                                  'total_loss_itr_mean': total_loss_itr_mean,
