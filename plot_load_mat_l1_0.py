...
psnr_nonsparse = []
sparsity = []
for filename in glob.glob("DCS_MNIST\MNIST_NL\mat_reg\FinalL1*"):
    dict_mat = loadmat(filename)
    meas = int(filename[-17:-15])
    spr = int(filename[-14:-11])
    sparsity.append(spr)

    total_loss_Sparse_itr_mean = dict_mat.get('total_loss_Sparse_itr_mean')
    total_loss_l1_itr_mean = dict_mat.get('total_loss_l1_itr_mean')
    total_loss_itr_mean = dict_mat.get('total_loss_itr_mean')
    sparseDCS_reconstloss = dict_mat.get('sparseDCS_reconstloss')
    l1DCS_reconstloss = dict_mat.get('l1DCS_reconstloss')
    ...
    l1DCS_test_reconstloss = dict_mat.get('l1DCS_test_reconstloss')
    ...
    reconstructions_l1_tst_np_reshape = dict_mat.get('reconstructions_l1_tst_np_reshape')

    reconstructions_Sparse_tst_np_reshape = np.reshape(reconstructions_Sparse_tst_np_reshape,[data_np_reshape.shape[0],data_np_reshape.shape[1]])
    reconstructions_l1_tst_np_reshape = np.reshape(reconstructions_l1_tst_np_reshape,
                                                       [data_np_reshape.shape[0], data_np_reshape.shape[1]])
    reconstructions_tst_np_reshape = np.reshape(reconstructions_tst_np_reshape,[data_np_reshape.shape[0],data_np_reshape.shape[1]])
    data_np_reshape = np.reshape(data_np_reshape, [data_np_reshape.shape[0], data_np_reshape.shape[1]])
    # # reconstructions_Sparse_tst_np_reshape.reshape([-1, reconstructions_Sparse_tst_np_reshape.shape[1]])
    ...
    l1DCS_mse = np.mean(np.square(data_np_reshape - reconstructions_l1_tst_np_reshape))
    DCS_mse = np.mean(np.square(data_np_reshape - reconstructions_tst_np_reshape))
    # # data_psnr = 10*np.log10(1/data_mse)
    l1DCS_psnr = 10 * np.log10(1 / l1DCS_mse)
    sparseDCS_psnr = 10 * np.log10(1 / sparseDCS_mse)
    DCS_psnr = 10 * np.log10(1 / DCS_mse)

    psnr_sparse.append(sparseDCS_psnr)
    psnr_l1.append(l1DCS_psnr)
    psnr_nonsparse.append(DCS_psnr)
    print('l1DCS_PSNR of measuremnt %d and sparsity %d = %f' % (meas,spr,l1DCS_psnr))
    ...
    # # SSIM
    l1DCS_ssim = ssim(data_np_reshape, reconstructions_l1_tst_np_reshape,
                          data_range=reconstructions_l1_tst_np_reshape.max()
                                     - reconstructions_l1_tst_np_reshape.min())
    ...
    #
    print('l1DCS_SSIM of measuremnt %d and sparsity %d = %f' % (meas,spr,l1DCS_ssim))
    ...
    # # Mean Ssuare Error
    l1DCS_mse = np.linalg.norm(data_np_reshape - reconstructions_l1_tst_np_reshape)
    ...
    print('l1DCS_MSE of measuremnt %d and sparsity %d = %f' % (meas,spr,l1DCS_mse))
    ...
    ax_hnd3, = ax.plot(20*np.log10(np.reshape(l1DCS_reconstloss,-1)))
    ax.legend((ax_hnd2, ax_hnd3), ( 'DCS Reconstruction error',
                                            'l1 Reconstruction error'), loc='upper right', shadow=True)
    ...
    plt.savefig(os.path.join('DCS_MNIST\MNIST_NL\plots\lp', save_title + '.png'),
        bbox_inches='tight')
    plt.show()
    ...
    ax_hnd3, = ax.plot(20 * np.log10(np.reshape(total_loss_l1_itr_mean, -1)))
    ax.legend((ax_hnd2, ax_hnd3), ('DCS: Total Loss','l1: Total Loss'), loc='upper right',
              shadow=True)
    ...
    #
    # ax_hnd1, = ax.plot(20 * np.log10(np.reshape(total_loss_itr_mean, -1)))
    # ax_hnd2, = ax.plot(20 * np.log10(np.reshape(total_loss_Sparse_itr_mean, -1)))
    # ax_hnd3, = ax.plot(20 * np.log10(np.reshape(total_loss_l1_itr_mean, -1)))
    # ax.legend((ax_hnd1, ax_hnd2, ax_hnd3), ('DCS: Total Loss', 'l0: Total Loss', 'l1: Total Loss'), loc='upper right',
    #           shadow=True)
    #
    # fig_title = ('Total Loss (meas = %d) ' % (meas))
    # ax.set_title(fig_title)
    # ax.set_ylabel('Total Loss [dB]')
    # ax.set_xlabel('epochs')
    # # ax.grid(which='major', alpha=0.5)
    ...
    # # plt.grid(color='b', linestyle='-', linewidth=1,which='minor', alpha=0.2)
    # plt.grid(True)
    # save_title = ('Total_Loss_%d' % (meas))
    # plt.savefig(os.path.join('DCS_MNIST\MNIST_NL\saved_mat_reg', save_title + '.png'),
    #             bbox_inches='tight')
    # plt.show()
#
# fig3, ax = plt.subplots()
# sparsity_a = np.array(sparsity)
# psnr_sparse_a = np.array(psnr_sparse)
# psnr_nonsparse_a = np.array(psnr_nonsparse)
# idx = np.argsort(sparsity_a)
# my_xticks = ['10','25','50','75','100','150','200','300','400','500','784']
# # ax.set_xticklabels(my_xticks, rotation=0, fontsize=5)
# plt.xticks(sparsity_a[idx], my_xticks,fontsize=6)
#
#
# ax_hnd1, = ax.plot(sparsity_a[idx],psnr_sparse_a[idx])
# # ax_hnd2, = ax.plot(sparsity_a[idx],psnr_nonsparse_a[idx])
# # ax.legend(( ax_hnd1,ax_hnd2), ('ProxDCS: PSNR', 'DCS: PSNR'), loc='upper right',
# #               shadow=True)
#
# fig_title = ('PSNR vs Support of latent space(measurement m = %d)' %(meas))
# ax.set_title(fig_title)
# ax.set_ylabel('PSNR(dB)')
# ax.set_xlabel('Latent support')
#     # ax.grid(which='major', alpha=0.5)
#
#     # plt.grid(color='b', linestyle='-', linewidth=1,which='minor', alpha=0.2)
# plt.grid(True)
# save_title = ('PSNR_support')
