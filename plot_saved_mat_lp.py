...
import glob
import os
for filename in glob.glob("D:\VEGA\MNIST\mnist_Lin_mcpLpz5_m25_spr100_lambda1\saved_var_25_784_100*"):
    ...
    total_loss_l0_itr_mean = dict_mat.get('total_loss_l0_itr_mean')
    total_loss_l1_itr_mean = dict_mat.get('total_loss_l1_itr_mean')
    total_loss_lp_itr_mean = dict_mat.get('total_loss_lp_itr_mean')
    total_loss_mcp_itr_mean = dict_mat.get('total_loss_mcp_itr_mean')

    sparseDCS_reconstloss = dict_mat.get('sparseDCS_reconstloss')
    DCS_reconstloss = dict_mat.get('DCS_reconstloss')
    l0DCS_reconstloss = dict_mat.get('l0DCS_reconstloss')
    l1DCS_reconstloss = dict_mat.get('l1DCS_reconstloss')
    lpDCS_reconstloss = dict_mat.get('lpDCS_reconstloss')
    mcpDCS_reconstloss = dict_mat.get('mcpDCS_reconstloss')
    ...
    # ax_hnd3, = ax.plot(20*np.log10(np.reshape(l0DCS_reconstloss,-1)))
    ax_hnd4, = ax.plot(20*np.log10(np.reshape(l1DCS_reconstloss,-1)))
    ax_hnd5, = ax.plot(20*np.log10(np.reshape(lpDCS_reconstloss,-1)))
    ax_hnd6, = ax.plot(20*np.log10(np.reshape(mcpDCS_reconstloss,-1)))
    ax.legend((ax_hnd1, ax_hnd2, ax_hnd4,ax_hnd5, ax_hnd6), ['l0:s-Sparse', 'DCS', 'l1', 'lp', 'mcp'],
              loc='upper left', fontsize= 'x-small', shadow=True)
    # fig_title = ('RE (latent space with s = %d and m = %d) ' %(spr,meas))
    fig_title = ('RE (Measurement:m = % d)' % (meas))
    ...
    plt.grid(True)
    save_title = ('RE_m_%d_spr_%d' % (meas,spr))
    plt.savefig(os.path.join('D:\VEGA\MNIST\mnist_Lin_mcpLpz5_m25_spr100_lambda1', save_title + '.png'),
        bbox_inches='tight')
    plt.show()
    ...
    # ax_hnd3, = ax.plot(20 * np.log10(np.reshape(total_loss_l0_itr_mean, -1)))
    ax_hnd4, = ax.plot(20 * np.log10(np.reshape(total_loss_l1_itr_mean, -1)))
    ax_hnd5, = ax.plot(20 * np.log10(np.reshape(total_loss_lp_itr_mean, -1)))
    ax_hnd6, = ax.plot(20 * np.log10(np.reshape(total_loss_mcp_itr_mean, -1)))
    ax.legend((ax_hnd1, ax_hnd2, ax_hnd4, ax_hnd5, ax_hnd6), ('l0:s-Sparse','DCS', 'l1', 'lp', 'mcp'),
                                                                loc='upper left',fontsize= 'x-small', shadow=True)

    # fig_title = ('Total Loss (latent space with s = %d and m = %d) ' % (spr, meas))
    fig_title = ('Total Loss (Measurement:m = %d) ' % (meas))
    ax.set_title(fig_title)
