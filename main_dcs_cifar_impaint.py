...
import nets_keras_cifar_noise as nets
...
def gaussian_noise_layer(input_layer, std):
    mask = np.ones([input_layer[1],input_layer[2]])
    mask[12:15,12:15] = 0
    mask_tf = tf.identity(mask)
    mask_tf = tf.reshape([1,input_layer[1],input_layer[2],1])
    mask_tf = tf.repeat(mask_tf,repeat = input_layer[0],axis = 0)
    mask_tf = tf.repeat(mask_tf, repeat=input_layer[3], axis=3)
    out = tf.math.multiply(input_layer,mask_tf)
...
def optimization_loss(meas_img_Sparse, optimized_sample_Sparse, gaussian_noise_layer):
    return tf.reduce_mean(gen_loss_fn(meas_img_Sparse, optimized_sample_Sparse, gaussian_noise_layer))
...
if load_weights == 1:
    ...
    mat_file = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/saved_var_%d_%d_%d.mat' % (
    ...
    generator_net_file = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/gen_n_%d_%d_%d_%d' % (
    ...
    generatorSparse_net_file = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/genSpar_n_%d_%d_%d_%d' % (
    num_measurements, num_latents, num_measurements, num_latents, dim_latent, load_epoch)
...
for epoch in range(200, epochs):
    ...
    for step in range(bat_per_epo):
        ...
        with tf.GradientTape() as tapeSparse:
            ...
            generator_loss_Sparse = optimization_loss(meas_img_Sparse, optimized_sample_Sparse, gaussian_noise_layer)
    ...
    # Model Saving
    if (epoch + 1) % 10 == 0:
        filename1 = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/gen_n_%d_%d_%d_%d' % (
        num_measurements, num_latents, num_measurements, num_latents, dim_latent, epoch)
        filename2 = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/meas_n_%d_%d_%d_%d' % (
        num_measurements, num_latents, num_measurements, num_latents, dim_latent, epoch)
        filename3 = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/genSpar_n_%d_%d_%d_%d' % (
        num_measurements, num_latents, num_measurements, num_latents, dim_latent, epoch)
        filename4 = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/measSpar_n_%d_%d_%d_%d' % (
        num_measurements, num_latents, num_measurements, num_latents, dim_latent, epoch)
...
mat_file = os.getcwd() + '/DeepcsTF2DN_m%d_spr%d/gen_n_%d_%d_%d.mat' % (
num_measurements, num_latents, num_measurements, num_latents, dim_latent)
